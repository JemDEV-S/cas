...
class InterviewEvaluationReportService
{
    const MIN_PASSING_SCORE = 35;
    const MAX_SCORE = 50;

    /**
     * Obtener postulaciones con evaluaciones de entrevista completadas
     */
    public function getEvaluatedApplications(string $jobPostingId): Collection
    {
        ...
        // Obtener la fase de entrevista (Fase 8)
        $interviewPhase = \Modules\JobPosting\Entities\ProcessPhase::where('code', 'PHASE_08_INTERVIEW')
            ->first();
        ...
        // Cargar evaluaciones de entrevista y evaluadores
        $applications->load(['evaluations' => function ($query) use ($interviewPhase) {
            $query->where('phase_id', $interviewPhase->id)
                  ->whereIn('status', [
        ...
        foreach ($applications as $application) {
            $interviewEvaluation = $application->evaluations->first();

            $application->evaluator_name = $interviewEvaluation?->evaluator?->name ?? 'N/A';
            $application->evaluation_comments = $interviewEvaluation?->general_comments ?? '';
        }
    ...
    public function organizeByStructure(Collection $applications): array
    {
        ...
        foreach ($structure as &$unit) {
            ...
            foreach ($unit['profiles'] as &$profile) {
                ...
                // Ordenar postulaciones por puntaje con bonus descendente
                usort($profile['applications'], function ($a, $b) {
                    $scoreA = $a->interview_score_with_bonus ?? $a->interview_score ?? 0;
                    $scoreB = $b->interview_score_with_bonus ?? $b->interview_score ?? 0;
    ...
    public function generateReport(JobPosting $jobPosting): array
    {
        ...
        return [
            'posting' => $jobPosting,
            'title' => 'RESULTADO DE EVALUACION DE ENTREVISTA PERSONAL',
            'subtitle' => $jobPosting->title ?? 'Proceso de Seleccion CAS ' . $jobPosting->year,
            'phase' => 'Entrevista Personal - Fase 8',
    ...
    public function generatePdf(JobPosting $jobPosting, ?string $outputPath = null): string
    {
        ...
        $html = View::make('document::templates.result_interview_evaluation_mdsj', $reportData)->render();
        ...
        $fileName = sprintf(
            'Resultado_Entrevista_%s_%s.pdf',
            str_replace(['/', '\\', ' '], '_', $jobPosting->code),
            now()->format('Y-m-d_His')
        );

        $directory = $outputPath ?? storage_path('app/reports/interview-evaluation');
        if (!is_dir($directory)) {
