...
class RegenerateJobProfileDocument
{
    ...
    /**
     * Maneja el evento JobProfileUpdated y regenera el documento oficial
     * Solo se regenera si el perfil está aprobado
     */
    public function handle(JobProfileUpdated $event): void
    {
        try {
            $jobProfile = $event->jobProfile;

            // Solo regenerar si el perfil está aprobado
            if ($jobProfile->status !== 'approved') {
                Log::info('Perfil no está aprobado, no se regenera documento', [
            ...
            // Buscar si ya existe un documento para este perfil
            ...
            // Preparar datos para el documento (usando el mismo método del listener original)
            $data = $this->prepareDocumentData($jobProfile);

            if ($existingDocument) {
                // Regenerar el documento existente
                Log::info('Regenerando documento existente', [
                    'job_profile_id' => $jobProfile->id,
                    'document_id' => $existingDocument->id,
                ]);

                $document = $this->documentService->regenerateDocument(
                    $existingDocument,
                    $data
                );
                ...
                // Generar nuevo documento si no existe
                Log::info('Generando nuevo documento', [
                    'job_profile_id' => $jobProfile->id,
                ]);
